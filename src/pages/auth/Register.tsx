...
export function Register() {
  ...
  useEffect(() => {
    publicService
      .listBusinessTypes()
      .then((b) => setBusinessTypes(b.data))
      .catch(() => {});
    publicService
      .listCategories()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (role === "Buyer") {
        ...
        localStorage.setItem("tawreed_token", token);
        localStorage.setItem("tawreed_refresh", refreshToken);
        localStorage.setItem("tawreed_user", JSON.stringify(user));
    ...
    <form onSubmit={handleSubmit} className=" m-6 ">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {[
          ...
          >
            <Icon
              className={`w-6 h-6 ${role === r ? "text-indigo-600" : "text-slate-400"}`}
            />
            <span className="text-xs font-extrabold">
              {t(r.toLowerCase() as any)}
            </span>
            <span className="text-[10px] opacity-70">{t(descKey)}</span>
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        ...
        {error && (
          <p className="text-xs font-medium text-red-600 text-center bg-red-50 py-2.5 rounded-lg">
            {error}
          </p>
        )}
