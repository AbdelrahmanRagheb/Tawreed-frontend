...
export function AdminUsers() {
  ...
  const [selectedBuyer, setSelectedBuyer] = useState<AdminUserDetail | null>(
    null,
  );
  ...
  useEffect(() => {
    const isModalOpen = Boolean(
      selectedBuyer || detailLoading || confirmTarget,
    );
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        />
        <span
          className={`text-xs font-medium ${isActive ? "text-emerald-700" : "text-red-700"}`}
        >
          {status}
        </span>
      </div>
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {toArabicNumeral(String(buyers.length), language)} {t("buyers")}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <p className="text-xl font-bold text-slate-900">
            {toArabicNumeral(buyers.length.toLocaleString(), language)}
          </p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <p className="text-xl font-bold text-slate-900">
            {toArabicNumeral(activeCount.toLocaleString(), language)}
          </p>
          <p className="text-[11px] text-slate-500 mt-0.5">
            {t("activeBuyers")}
          </p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <p className="text-xl font-bold text-slate-900">
            {toArabicNumeral(suspendedCount.toLocaleString(), language)}
          </p>
          <p className="text-[11px] text-slate-500 mt-0.5">
            {t("suspendedBuyers")}
          </p>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6">
                      <div className="bg-slate-50 rounded-xl p-5">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="flex items-center gap-2 text-sm text-slate-600">
                            ...
                            <span>
                              {t("joined")}{" "}
                              {formatDate(selectedBuyer.joinedDate)}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-sm text-slate-600">
                            ...
                            <span>
                              {t("lastLogin")}{" "}
                              {formatDate(selectedBuyer.lastLoginAt)}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-sm text-slate-600">
                      ...
                      <div className="bg-slate-50 rounded-xl p-5">
                        ...
                        {selectedBuyer.status === "Suspended" &&
                          selectedBuyer.suspensionReason && (
                          ...
                          {[
                            {
                              ...
                              value: selectedBuyer.ordersCreated,
                              color: "text-indigo-600",
                              bg: "bg-indigo-100",
                            },
                            {
                              icon: Users,
                              label: t("ordersJoined"),
                              value: selectedBuyer.ordersJoined,
                              color: "text-blue-600",
                              bg: "bg-blue-100",
                            },
                            {
                              ...
                              value: selectedBuyer.completedOrders,
                            ...
                            },
                            {
                              ...
                              value: selectedBuyer.cancelledOrders,
                              color: "text-red-600",
                              bg: "bg-red-100",
                            },
                          ].map((stat) => (
                              ...
                              <tbody className="divide-y divide-slate-100">
                                {selectedBuyer.recentOrders.map((order) => (
                                  <tr key={order.id}>
                                    ...
                                    <td className="px-4 py-2.5 text-xs text-slate-700 text-end font-medium">
                                      {formatCurrency(order.estimatedTotal)}
                                    </td>
                                    <td className="px-4 py-2.5 text-xs text-slate-700 text-end">
                                      {toArabicNumeral(
                                        String(order.participantsCount),
                                        language,
                                      )}
                                    </td>
                                    <td className="px-4 py-2.5 text-xs text-slate-500 text-end">
      ...
      {confirmTarget && (
        <div
          className="fixed inset-0 z-[80] overflow-y-auto bottom-16 md:bottom-0"
          onClick={() => {
            setConfirmTarget(null);
            setSuspendReason("");
          }}
          ...
          <div className="relative flex min-h-full w-full items-center justify-center p-4 pb-[calc(6rem+env(safe-area-inset-bottom))] sm:p-4 sm:pb-4">
            ...
            >
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button
                  onClick={() =>
                    toggleStatus(confirmTarget.id, confirmTarget.action)
                  }
