...
import {
  ...
  MapPin, Calendar, UserPlus, LogOut, Activity, AlertCircle, CheckCircle,
} from 'lucide-react';
...
export function OrderDetail() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || err?.message || 'Failed to join order');
    } finally {
      setJoining(false);
    }
  };
  ...
  const handleLeave = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || err?.message || 'Failed to leave order');
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-500">
              ...
              <span className="flex items-center gap-1">
                <Calendar className="w-3 h-3" />{t('deadline')}: {new Date(order.deadline).toLocaleString(language === 'ar' ? 'ar-SA' : 'en-US')}
              </span>
            </div>
          </div>
          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${
            order.status === 'Open' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' :
            order.status === 'Closed' ? 'bg-slate-100 border-slate-300 text-slate-600' :
              ...
              {order.activities.slice(-6).reverse().map((act) => (
                <div key={act.id} className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">{new Date(act.createdAt).toLocaleString(language === 'ar' ? 'ar-SA' : 'en-US')}</p>
